...
- **`ps` (Process Status)**：静态查看进程快照。
  ...
  | STAT 基本状态 | 描述         | STAT状态符号 | 描述                                       |
  | ------------- | ------------ | ------------ | ------------------------------------------ |
  | R             | 进程运行     | s            | 进程是控制进程，Ss：进程的领导者，父进程   |
  | S             | 可中断睡眠   | <            | 进程运行在高优先级上，S<：优先级较高的进程 |
  | T             | 进程被暂停   | N            | 进程运行在低优先级上，SN：优先级较低的进程 |
  | D             | 不可中断睡眠 | +            | 进程运行在前台，R+：前台运行的进程         |
  | Z             | 僵尸进程     | l            | 进程是多线程的，SI：进程是以线程方式运行   |
...
- **`top` / `htop`**：动态、交互式地实时查看进程和系统资源占用情况
  ...
  | 核心状态名称 | 描述                            |      |                       |
  | ------------ | ------------------------------- | ---- | --------------------- |
  | us           | 用户态占用CPU百分比             | wa   | CPU等待IO完成的时间   |
  | sy           | 内核态占用CPU百分比             | hi   | 硬中断，占用CPU百分比 |
  | ni           | 调整过优先级的程序CPU占用百分比 | si   | 软中断，占用CPU百分比 |
  | id           | 剩余CPU百分比                   | st   | 虚拟化工具占用        |
