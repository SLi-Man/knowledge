...
::: tip 笔试题：如果默认创建000属性的文件请问UMASK值是多少？{#interviewQuestion02}
答：是 666。
:::
...
特殊权限位是 12 个权限位中的前 3 位，可以通过 [stat](Basic_Commands.md#stat) 命令查看，设置方法与普通权限相同。

- `suid` ：4，给文件添加 s 权限后，所有使用这个命令的用户都拥有该命令主人的权限。针对二进制命令或程序，不能作用在脚本上。
- `sgid` ：2，基本和suid相同，所有使用这个命令的用户都拥有命令主人**所属组**的权限。
